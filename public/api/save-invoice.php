...
try {
    ...
    // Instellingen laden voor supplier snapshot
    ...
    $vatReason = $data['vat_exempt_reason'] ?? null;
    ...
    if ($buyerReference === '') {
        $buyerReference = $paymentReference !== '' ? $paymentReference : null;
    }
    ...
    $insertInvoice = $pdo->prepare("INSERT INTO invoices (
        ...
        status, total_excl, total_vat, total_incl, vat_rate, vat_exempt, vat_exempt_reason,
        payment_reference, payment_terms, buyer_reference, supplier_snapshot, customer_snapshot
    ) VALUES (
        NULL, :client_id, :user_id, :invoice_number, :issue_date, :due_date, :currency_code,
        :status, :total_excl, :total_vat, :total_incl, :vat_rate, :vat_exempt, :vat_exempt_reason,
        :payment_reference, :payment_terms, :buyer_reference, :supplier_snapshot, :customer_snapshot
    )");
    ...
    $insertInvoice->execute([
        ...
        ':vat_rate' => $vatExempt ? 0 : $defaultVatRate,
    ...
    echo json_encode(['error' => 'Fout bij opslaan factuur: ' . $e->getMessage()]);
}
